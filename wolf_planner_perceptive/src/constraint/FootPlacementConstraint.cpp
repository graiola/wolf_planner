...
bool FootPlacementConstraint::isActive(scalar_t time) const {
  return dynamic_cast<const PerceptivePlannerReferenceManager&>(*referenceManagerPtr_).getFootPlacementFlags(time)[contactPointIndex_];
}

vector_t FootPlacementConstraint::getValue(scalar_t /*time*/, const vector_t& state, const PreComputation& preComp) const {
  const auto param = cast<PerceptivePlannerPreComputation>(preComp).getFootPlacementConParameters()[contactPointIndex_];
